...
class eon_sale_order(models.Model):
    ...
    invoice_state = fields.Selection([('draft', 'Draft'),
                                    ...
                                    ('proforma2', 'Pro-forma'),
                                    ('open', 'Open'), ('paid', 'Paid'),
                                    ('cancel', 'Cancelled'), ], string=' Invoice Status', index=True, readonly=True, default='draft')
    picking_state = fields.Selection([('draft', 'New'),
                                    ('cancel', 'Cancelled'),
                                    ('waiting', 'Waiting Another Move'),
                                    ('confirmed', 'Waiting Availability'),
                                    ('assigned', 'Ready To Transfer'),
                                    ('partially_availible', 'Partial Available'),
                                    ('done', 'Done')
    ...
    sale_order_lines = fields.One2many('sale.order.line', 'order_id', 'Order Lines', readonly=True)
...
class sale_order(models.Model):
        ...
        if self.id:
            ...
            for picking in self.picking_ids:
                for move_line in picking.move_lines:
                    ...
                    self.env['eon.stock.move.line'].create(vals)
    ...
    def action_cancel(self):
        ...
        for rec in self:
            eon_account_obj.browse(rec.sale_order_id.id).write({'order_state': 'cancel', 'picking_state': 'cancel'})
        return res
    ...
    def action_ignore_delivery_exception(self):
        ...
        if self.sale_order_id:
            self.sale_order_id.write({'order_state': 'progress'})
        return res
    ...
    def create(self, vals):
        ...
        rec_vals.update({'name': vals.get('name', False), 'partner_id': vals.get('partner_id', False),
                    'partner_invoice_id': vals.get('partner_invoice_id', False),
                     'due_date':res_vals.validity_date,
    ...
    @api.multi
    def write(self, vals):
        res_vals = super(sale_order, self).write(vals)
        rec_vals = {}
        if vals.get('validity_date',False):
            rec_vals.update({'due_date': vals.get('validity_date',False)})
        sale_rec_id = self.env['eon.sale.order'].write(rec_vals)
        return res_vals
